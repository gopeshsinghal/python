...
from random_words import pick_random_word
...
def input_character_in_words(selected_word, input_char, current_word_state, attempts_remaining):
    ...
    if input_char in selected_word:
        current_word_state = change_current_word_state(
            selected_word, current_word_state, input_char)
    else:
...
def play_game(attempt=5):
    ...
    while True:
        ...
        current_word_state, attempts_remaining = input_character_in_words(
            selected_word, input_char, current_word_state, attempts_remaining)

        print_current_state(current_word_state, attempts_remaining)

        game_ended = check_game_status(
...
while intrust:
    ...
    k = input('wanna play again..?\n')
    yes = ['sure', 'yes', 'yeah', 'yep', 'yaa', 'ha', 'haa', 'haan',
           'why not', 'ji bilkul', 'haan ji', 'haanji', 'hnnji', 'hnn', 'hambe']
    no = ['no', 'noooo', 'nope', 'eww', 'naah', 'na', 'naa',
          'never', 'sorry', 'nahi', 'ghanta', 'bhagg bsdk']
